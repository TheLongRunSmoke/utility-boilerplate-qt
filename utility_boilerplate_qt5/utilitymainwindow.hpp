 ...
 * handling events (including settings change), interface to read and write working file.
 *
 * Using:
 * 1) Inherit from UtilityMainWindow.
 * 2) Initialize UI in constructor, by Qt documentation. Add you widget for example.
 * 2) Override loadFile(fileName) and saveFile(filename) implementing read and write file from disc.
 *      See example for basic variant.
 * 3) Override getExtensions(), getFileReadMode(), getFileWriteMode() to specified file types and read-write modes.
 * 4) Delegate cut(), copy(), paste() to your widget.
 *
...
public:
    ...
    /**
     * Handle loading of specified file. Reading from disc must implemented in heir.
     */
     ...
     *
     * SettingsDialog use object name to determinate what entity must received SettingsChange event.
     */
...
protected:
    ...
    /**
     * Handle close event to ask user to save document if isDocumentModified() is true.
     */
    ...
    /**
     * Populate File menu and toolbar with actions like open, save, recent files and so on.
     */
    ...
    /**
     * File read mode from QFile, use to specified how Qt must read file from disc.
     */
    virtual QIODevice::OpenMode getFileReadMode();

    /**
    * File write mode from QFile, use to specified how Qt must write file to disc.
    */
     ...
     * @param toolbar pointer to toolbar object where action be add. Ignored if nullptr.
...
protected slots:
    ...
    /**
     * Open document from recent list. Current document can be interactively saved.
     */
    void openRecentFile();
    ...
    /**
     * Cut selection from active widget in to clipboard. Must be implemented first.
     */
    virtual void cut();

    /**
     * Copy selection from active widget in to clipboard. Must be implemented first.
     */
    virtual void copy();

    /**
     * Paste from clipboard to selections in active widget. Must be implemented first.
     */
    virtual void paste();
...
private:
    ...
    /**
     * Toolbars. Keep pointer as class member to make possible to change translate on-fly.
     */
