...
#    if !defined(NVWA_USE_CXX11_MUTEX) && HAVE_CXX11_MUTEX != 0 && !defined(_WIN32THREADS)    \
        && !defined(NVWA_WIN32THREADS) && !defined(NVWA_PTHREADS) && !defined(NVWA_NOTHREADS) \
        && defined(_WIN32) && defined(_MT) && (!defined(_MSC_VER) || defined(_DLL))
...
#    if !defined(_WIN32THREADS) && (defined(_WIN32) && defined(_MT))
...
#    if HAVE_CXX11_MUTEX != 0 && !defined(_NOTHREADS) && !defined(NVWA_NOTHREADS)    \
        && !defined(_PTHREADS) && !defined(NVWA_PTHREADS) && !defined(_WIN32THREADS) \
...
#if defined(_NOTHREADS)
#    if defined(_PTHREADS) || defined(_WIN32THREADS) || NVWA_USE_CXX11_MUTEX != 0
#        undef _NOTHREADS
...
#if NVWA_USE_CXX11_MUTEX == 0 && !defined(NVWA_WIN32THREADS) && !defined(NVWA_PTHREADS) \
...
#    endif
    ...
    ~fast_mutex() { _FAST_MUTEX_ASSERT(!_M_locked, "~fast_mutex(): still locked"); }
    void lock() {
#    ifdef _DEBUG
        _FAST_MUTEX_ASSERT(!_M_locked, "lock(): already locked");
        _M_locked = true;
