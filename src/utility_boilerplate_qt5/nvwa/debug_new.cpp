...
#    else
#        define _DEBUG_NEW_FILENAME_LEN 120
#    endif
#endif
...
enum is_array_t { alloc_is_not_array, alloc_is_array };
...
namespace {
     ...
     * @return           aligned size
     */
    inline constexpr uint32_t align(size_t s, size_t alignment = _DEBUG_NEW_ALIGNMENT) {
     ...
     */
    new_ptr_list_t new_ptr_list = {&new_ptr_list,
                                   &new_ptr_list,
                                   0,
                                   {
#if _DEBUG_NEW_FILENAME_LEN == 0
...
#    elif NVWA_CYGWIN || (NVWA_WIN32 && defined(WINVER) && WINVER >= 0x0500)
...
#    if NVWA_WINDOWS
            if (len <= 4
                || (strcmp(cmd + len - 4, ".exe") != 0 && strcmp(cmd + len - 4, ".EXE") != 0)) {
                strcpy(cmd + len, ".exe");
                len += 4;
            }
#    endif
...
#    endif
    ...
    void free_pointer(void* usr_ptr, void* addr, is_array_t is_array,
        ...
        if (is_array != ptr->is_array) {
            ...
            fast_mutex_autolock lock(new_output_lock);
            fprintf(new_output_fp, "%s: pointer %p (size %zu)\n\tat ", msg, usr_ptr, ptr->size);
            print_position(addr, 0);
...
int check_leaks() {
    ...
    while (ptr != &new_ptr_list) {
...
#if _DEBUG_NEW_TAILCHECK
        if (!check_tail(ptr)) {
            fprintf(new_output_fp, "warning: overwritten past end of object at %p\n", usr_ptr);
        }
#endif
        ...
        if (is_leak_whitelisted(ptr)) {
            ...
            fprintf(new_output_fp, "Leaked object at %p (size %zu, ", usr_ptr, ptr->size);
...
int check_mem_corruption() {
    ...
    for (new_ptr_list_t* ptr = new_ptr_list.next; ptr != &new_ptr_list; ptr = ptr->next) {
        ...
        if (ptr->magic != DEBUG_NEW_MAGIC) {
#endif
            fprintf(new_output_fp, "Heap data corrupt near %p (size %zu, ", usr_ptr, ptr->size);
#if _DEBUG_NEW_TAILCHECK
        } else {
            // Adjust usr_ptr after the basic sanity check
            usr_ptr = reinterpret_cast<const char*>(ptr) + ptr->head_size;
...
#if _DEBUG_NEW_REMEMBER_STACK_TRACE
        if (ptr->stacktrace != nullptr) print_stacktrace(ptr->stacktrace);
#endif
...
void debug_new_recorder::_M_process(void* usr_ptr) {
    ...
    ptr->file = _M_file;
#else
    strncpy(ptr->file, _M_file, _DEBUG_NEW_FILENAME_LEN - 1)[_DEBUG_NEW_FILENAME_LEN - 1] = '\0';
#endif
    ptr->line = _M_line;
#if _DEBUG_NEW_REMEMBER_STACK_TRACE == 2
    free(ptr->stacktrace);
 ...
 */
debug_new_counter::debug_new_counter() { ++_S_count; }
